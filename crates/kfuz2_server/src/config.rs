...
///
/// TODO: Replace this stub with actual file reading and parsing logic.
pub async fn load_config(_: &str) -> Result<ConfigData, Box<dyn std::error::Error>> {
    let filename = "test.toml";
    let mut contents = (fs::read_to_string(filename).await).unwrap_or_else(|_| String::new());
    ...
    let data: ConfigData = match toml::from_str(&contents) {
        ...
        Err(err) => {
            println!(
                "Unable to load data from `{filename}`, error: {err}. Created new file for you! Go fill the fields by your taste."
            );
